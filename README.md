...
## 1.1) Як запускати з GitHub без постійного скачування ZIP

Один раз клонувати репозиторій:
...
Далі при кожному оновленні достатньо:

```bash
git pull
```

Після `git pull` запустіть бота з цієї ж папки:

```bash
python bot.py
```

> Важливо: `.env` з вашим токеном зберігається локально і не перезаписується при `git pull`, якщо ви не комітили `.env` у репозиторій.

### Швидкий щоденний сценарій
...
### Щоб бот працював постійно (24/7) без ручного запуску

- **Linux-сервер/VPS**: запуск через `systemd` + `Restart=always`.
- **Хостинги**: Render / Railway / Fly.io (підключаєте GitHub repo, додаєте `TELEGRAM_BOT_TOKEN` як Secret/Environment Variable, і деплой іде автоматично після push).
